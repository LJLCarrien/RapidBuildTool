...
static public class NGUIEditorTools
{
        ...
        params GUILayoutOption[] options)
    {
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            {
                GUILayout.BeginHorizontal();
                if (GUILayout.Button(spriteName, "HelpBox", GUILayout.Height(18f)))
                {
                    LoacationSpriteAsset(atlas, spriteName);
                }
    ...
    static public void LoacationSpriteAsset(UIAtlas pAtlas, string pSpriteName)
    {
        if (pAtlas == null || string.IsNullOrEmpty(pSpriteName))
        {
            Debug.LogError("Project中找不到该图片" + pSpriteName);
            return;
        }
        EditorApplication.ExecuteMenuItem("Window/Project");
        var tPath1 = Application.dataPath;
        tPath1 = tPath1.Substring(0, tPath1.LastIndexOf('/'));
        var tPath2 = AssetDatabase.GetAssetPath(pAtlas);
        tPath2 = tPath2.Substring(0, tPath2.LastIndexOf('/'));

        var tPath = tPath1 + "/" + tPath2;
        var tFiles = System.IO.Directory.GetFiles(tPath, "*.png", System.IO.SearchOption.AllDirectories);
        foreach (var tFile in tFiles)
        {
            try
            {
                var tFileInfo = new System.IO.FileInfo(tFile);
                var tSpriteName = tFileInfo.Name.Replace(tFileInfo.Extension, String.Empty);
                if (tSpriteName == pSpriteName)
                {
                    var tNewPath = tFile.Replace(tPath1 + "/", String.Empty);
                    var tSprite = AssetDatabase.LoadAssetAtPath(tNewPath, typeof(object));
                    if (tSprite != null)
                    {
                        EditorGUIUtility.PingObject(tSprite);
                    }
                    break;
                }
            }
